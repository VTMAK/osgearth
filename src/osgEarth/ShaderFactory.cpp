   ...
   std::vector< osg::ref_ptr<osg::Shader> >& out_shaders) const
{
   ...
   /*std::string vertdata;
   {
      ...
      std::string lastInterp, lastPrec;
      ...
      auto packWidth = [&](const Variable& v) -> int {
         if (v.arraySize > 0) return -1; // arrays are never packable
         if (v.type == "float") return 1;
         if (v.type == "vec2")  return 2;
         return -1;
         };

      auto locationsPerElement = [&](const Variable& v) -> int {
         if (v.type == "mat4") return 4;
         if (v.type == "mat3") return 3;
         return 1;
         };
      ...
      for (const Variable& v : vars)
      {
         ...
         // qualifier change => cannot share packing
         if (v.interp != lastInterp || v.prec != lastPrec)
            resetPacking();

         lastInterp = v.interp;
         lastPrec = v.prec;

         if (!v.interp.empty()) line << v.interp << " ";
         if (!v.prec.empty())   line << v.prec << " ";
         ...
         if (width > 0)
         {
            ...
            line << "layout(location=" << packLocation
         ...
         {
            // arrays, matrices, vec3+, etc.
            resetPacking();

            line << "layout(location=" << nextLocation << ") ";
   ...
   const auto build_vp_defines = [&vars](const std::string blockName) {
      std::stringstream buf;
      buf << "\n// Varying aliases\n";
      for (Variables::const_iterator i = vars.begin(); i != vars.end(); ++i)
      {
         buf << "#define " << i->name << " " << blockName << "." << i->name << "\n";
      }
      return buf.str();
   };*/
